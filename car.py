...
# Processamento após upload do CSV
if uploaded_file:
    ...
    for col in lista_colunas:
        ...
        stats = gerar_estatisticas(df, col, considerar_idle)
        tabela_final.append(stats)

    df_stats = pd.DataFrame(tabela_final)
    st.dataframe(df_stats, use_container_width=True)

# Continuação do app.py – Parte 3
...
def gerar_relatorio_txt(df, df_stats, distancia, consumo, kml, alertas):
    ...
    if alertas:
        ...
        st.markdown("- ✅ Nenhum alerta crítico identificado.")
    ...
    st.download_button(
        ...
        data=relatorio_txt,
        file_name="relatorio_forscan.txt",
        mime="text/plain"
    )
